...
const Module: React.FC<ModuleProps> = ({ module, onProgressUpdate }) => {
  ...
  useEffect(() => {
    // Avoid setting state if it's already the same to prevent loop/warning
    if (videoProgress !== module.progress) setVideoProgress(module.progress);
    if (videoWatched !== module.videoWatched) setVideoWatched(module.videoWatched);
    if (starsEarned !== module.starsEarned) setStarsEarned(module.starsEarned);
